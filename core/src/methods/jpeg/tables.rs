//! Standard baseline-JPEG tables (Annex K): quantization, zig-zag, and the
//! canonical Huffman specifications. We always encode with these, so encoder and
//! decoder agree.

/// Zig-zag scan order: `ZIGZAG[k]` = natural (row-major) index of the k-th
/// coefficient in zig-zag sequence.
...
/// A Huffman table specification: `counts[i]` = number of codes of length i+1
/// (i in 0..16), then the symbol values in code order.
pub struct HuffSpec {
    pub counts: [u8; 16],
